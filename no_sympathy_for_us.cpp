...
int &currentMatch, int &currentRound, int &step) {
    ...
    if (step == 8) {
        if (event.type == Event::MouseButtonPressed) {
            ...
            // Team 1 button click
            if (mousePos.x > 50 && mousePos.x < 200 && mousePos.y > 150 && mousePos.y < 200) {
                nextRoundTeams[currentMatch / 2] = knockoutTeams[currentMatch * 2];
                currentMatch++;
            }
            // Team 2 button click
            else if (mousePos.x > 250 && mousePos.x < 400 && mousePos.y > 150 && mousePos.y < 200) {
                nextRoundTeams[currentMatch / 2] = knockoutTeams[currentMatch * 2 + 1];
                currentMatch++;
            }
            ...
            if (currentMatch >= knockoutTeamsCount / 2) {
                ...
                if (currentRound >= log2(numTeams / 2)) {
                    step = 9;  // Move to Step 9 (final match)
                }
            }
        }
    }
